...
const WifiIcon: Component<WifiIconProps> = props => {
	...
	return (
		<svg
			...
			viewBox="0 0 33 24"
			fill="none"
			class={"w-6 h-6 tooltip_item cursor-pointer duration-150 dlcomponent"}
			onMouseUp={() => {
			...
			onMouseOver={() => {
				iconRef?.classList.add("scale-90");
			}}
			onFocus={() => {
				iconRef?.classList.add("scale-90");
			}}
...
export function WispMenu(props: WispMenuProps) {
	const [servers, setServers] = createSignal<Server[]>([]);
	const [loading, setLoading] = createSignal<boolean>(true);
	let menuRef: HTMLDivElement | undefined;
	const [isinDiag, setisinDiag] = createSignal(false);
		...
		const servers = await Promise.all(
			data.map(async server => {
				...
				return {
					...
					connected: server.id === settings.wispServer,
					isOnline: res.latency !== settings.wispServer,
				};
			}),
		);
		setServers(servers);
	...
	onMount(() => {
		...
		onCleanup(() => {
			window.removeEventListener("update-wispsrvs", fetchServers);
		});
	});
	...
	createEffect(() => {
		const leave = (e: MouseEvent) => {
			...
			if (e.button === 0) {
				if (!menuRef?.contains(e.target as Node) && !withinRadius(e)) {
					setTimeout(() => {
						if (!isinDiag()) {
	...
	async function connectToServer(server: Server) {
		const settings = await Filer.fs.promises.readFile(`/home/${await window.tb.user.username()}/settings.json`, "utf8");
	...
	return (
		<div
            ...
            ${props.isOpen ? "duration-150" : "opacity-0 pointer-events-none -translate-y-6 duration-200"}
        `}
			id="wisp-selector"
			ref={menuRef}
		>
			<div
                ...
                ${props.isOpen ? "" : "opacity-0 -translate-y-2"}
            `}
			>
				<div
					class={`
				...
				>
					<Show when={!loading()} fallback={<p>Loading...</p>}>
						<Show when={servers().length > 0} fallback={<span class="font-medium text-lg text-[#ffffffc8]">No servers found</span>}>
							<For each={servers()}>
								{server => (
									<Show when={server.isOnline}>
										<div
											class={`
										...
										>
											<div class="flex gap-3 items-center pointer-events-none">
												...
												<div class="flex flex-col">
													<h3 class="font-bold leading-tight">{server.name}</h3>
													<p class="font-medium text-xs leading-tight">{server.id}</p>
												</div>
											</div>
											<p class="text-sm p-2 bg-[#ffffff38] rounded-lg leading-none text-[#ffffffce] font-extrabold pointer-events-none">{server.latency}</p>
										</div>
									</Show>
								)}
							</For>
				...
				<div class="flex gap-2 w-full justify-center items-center">
					<button
						class="p-3 px-4 rounded-md bg-[#ffffff18] shadow-tb-border-shadow leading-none font-medium hover:bg-[#ffffff28] duration-150 cursor-pointer"
						type="button"
						onClick={() => {
							setisinDiag(true);
